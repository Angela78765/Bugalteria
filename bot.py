...
def regclose_markup():
    return {
        "inline_keyboard": [
            [{"text": "Регестрация ФОП", "callback_data": "reg_fop"}],
            [{"text": "Закрытие ФОП", "callback_data": "close_fop"}],
        ]
    }

def register_payment_inline():
    return {
        "inline_keyboard": [
            [{"text": "Регистрируем", "callback_data": "pay_reg_fop"}]
        ]
    }

def close_payment_inline():
    return {
        "inline_keyboard": [
            [{"text": "Закрываем", "callback_data": "pay_close_fop"}]
        ]
    }
...
WELCOME_SERVICES_TEXT = (
    "Вітаю\n"
    "Мене звати,  ——— !\n"
    "Я бухгалтер для ФОП — допомагаю підприємцям спокійно вести справи, не хвилюючись за податки, звітність і взаємодію з державними органами.\n"
...
BILL_INFO = (
    ...
    "❤️ОБОВ’ЯЗКОВО після здійснення оплати скиньте будь ласка чек або скрін екрану сюда @телеграм тег )"
)
...
def webhook():
    ...
    if "callback_query" in update:
        ...
        # ===== Новый сценарий: реєстрація / закриття =====
        if data == "regclose":
            send_message(chat_id, "Оберіть, будь ласка, послугу:", reply_markup=regclose_markup())
            return "ok", 200
        if data == "reg_fop":
            send_message(chat_id, REGISTER_FOP_TEXT, reply_markup=register_payment_inline())
            return "ok", 200
        if data == "pay_reg_fop":
            send_message(chat_id, BILL_INFO, parse_mode="HTML")
            return "ok", 200
        if data == "close_fop":
            send_message(chat_id, CLOSE_FOP_TEXT, reply_markup=close_payment_inline())
            return "ok", 200
        if data == "pay_close_fop":
            send_message(chat_id, BILL_INFO, parse_mode="HTML")
            return "ok", 200
        # ===== /Конец сценария регистрации/закрытия =====

        # Обработка сервисных инлайн-кнопок
        if data in ("consult", "support", "reports", "prro", "decret"):
    ...
    if text == "Реквізити оплати" and cid not in active_chats:
        send_message(cid, "<b>Реквізити для оплати:</b>\nПриватБанк: 1234 5678 0000 1111\nМоноБанк: 4444 5678 1234 5678\nIBAN: UA12 1234 5678 0000 1111 1234 5678", parse_mode="HTML", reply_markup=return_to_menu_markup())
        return "ok", 200
    # ======= Теперь кнопки меню обратно не трогаем, а просто ловим на inline =======
